...
class GenshinResinPromptingService(private val user: User) : AbstractCactusService() {
    ...
    init {
        this.serviceName = this.serviceName + "@(${user.genshinUID})"
    }

    override suspend fun main() {
        val setting = userSettings[user.id._value] ?: return

        if (setting.push && setting.resinRemind in 0..0)
    ...
    override fun toString(): String {
        return "$serviceName@${hashCode()}(${user.genshinUID})"
    }
}